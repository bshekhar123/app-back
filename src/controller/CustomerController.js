...
const { customerModel, addressMod } = require('../model/Customer');
...
const addAdress = async (req, res) => {
    try {
        ...
        const user = await customerModel.findById(_id);
        const { addressLine, city, state, country, pin } = req.body;
        const newAdd = new addressMod({ addressLine, city, state, country, pin });
        user.address.push(newAdd);
        await user.save()
...
const removeAdress = async (req, res) => {
    try {
        ...
        const user = await customerModel.findById(_id);
        const { id } = req.body;
        const filteredAdd = user.address.filter(e => e._id != id);
        user.address = filteredAdd;
        await user.save();
